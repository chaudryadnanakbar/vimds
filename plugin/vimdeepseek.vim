...
" API Key file path (default: ~/.config/gemini.token)
let s:api_key_file = expand('~/.config/vimds.token')

" Model name (default: gemini-3.6-flash)
let g:vimds_model = get(g:, 'vimds_model', 'modelname')
...
" Log file path
let g:vimds_log_file = get(g:, 'vimds_log_file', expand('~/.var/vimds/vimds.log'))

" Logging state ('on' or 'off')
let g:vimds_logging = get(g:, 'vimds_logging', 'off')

" Default path for saved reviews
let g:vimds_review_path = get(g:, 'vimds_review_path', '')

" Thinking state ('on' or 'off')
let g:vimds_thinking = get(g:, 'vimds_thinking', 'on')
...
" Optional: Add command completion for VimDS commands
function! s:VimdsComplete(ArgLead, CmdLine, CursorPos)
  let l:commands = ['VimdsListModels', 'VimdsChat', 'VimdsThinking', 
